...
**Entregado (ampliado el 2026-09-19 tras probarlo en QHD con `C:\`):** migas y conmutador de vista comparten fila; el nombre de cada tarjeta se muda a la fila que la filtra; el bloque de encabezado de la tarjeta de archivos se sustituye por una línea (la advertencia del ranking sin consulta, lo que cubre la búsqueda con ella); la ruta completa aparece cuando falla copiarla, no siempre; y las filas se desplazan en **su propia región**, con la cabecera de columnas pegada arriba, acotada a lo que queda de ventana, de modo que el conteo y la paginación no se van con la página. El límite descuenta también lo que va **después** de la región —su propio pie, el pie de la aplicación— y el panel del explorador se acota igual, porque una columna más alta que la ventana obliga a desplazar la página por muy corta que sea la otra. Resultado: **de FHD en adelante la página no se desplaza en absoluto**. Cuando quedarían menos de 150 px (unas tres filas) —o la ventana es compacta— el límite se retira y la página se desplaza como antes: es lo que ocurre en HD y con texto al 200 %, donde encima de las filas ya no queda sitio.
...
**Criterio fijado en las pruebas:** `SCREENS` en `tests/ui.spec.ts` ejecuta la misma aceptación en las cuatro pantallas con un mínimo por pantalla (HD ≥ 3 filas, FHD ≥ 9, QHD ≥ 16, UHD ≥ 20); donde la región se acota exige además el pie de la tabla y el de la aplicación a la vista y que la altura de página sea **exactamente** la de la ventana; y en todas, que no haya desplazamiento horizontal ni se pierdan el conmutador ni la búsqueda. Una prueba aparte reproduce el caso real —análisis parcial de una unidad completa, con su aviso y sus omitidos encima de las filas— en QHD. La garantía se escribe **por pantalla**, no como número de filas por página: ninguna de las cuatro comparte ese número.
