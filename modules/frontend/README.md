...
- Layout checks run the same acceptance at four screen sizes in CSS pixels — HD 1280×720, FHD 1920×1080, QHD 2560×1440 and UHD 3840×2160 — each with the rows it must show and its count in view, plus 390 px and 200% text. Windows scaling makes these cover the usual monitors: 4K at the default 200% is FHD in CSS pixels, and QHD at 150% falls between HD and FHD. Screenshots land in `test-results/workspace-<screen>.png`.
...
- Controls do not move when the view, the path or the filter changes: the view switch sits before the breadcrumbs, a deep path scrolls within its line, the count under a list keeps its height with or without pages, and the line explaining the list reserves room for the longer of its two sentences.
- Rows scroll inside their table, whose column header stays at the top of that region. The region is bounded to the window, counting the count and pagination under it, the page footer and the explorer column beside it, so at FHD and above the whole workspace fits and the page does not scroll at all; a window too short for a useful region (HD, or 200% text) keeps a single page scroll instead, and so do compact ones. The contents table filters the selected folder's direct items, by name and type, and sorts by name/size. Its pages contain 25 items; search result pages contain 50, and pages stop after the first 10,000 matches with a line asking to narrow the search. The tree is not paginated.
