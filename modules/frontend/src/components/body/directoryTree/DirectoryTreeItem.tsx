...
export const DirectoryTreeItem = ({
  directory,
  level,
  maxLevel,
  marginBottom,
  ...
  const getIconType = () => {
    ...
    if (
      ...
      (directory.subdirectories.length === 0 || level - 1 === maxLevel)
    )
  ...
  return (
    ...
    >
      <a
        ...
        onClick={() => {
          setShowChildrenItems(!showChildrenItems);
        }}
      >
        <IconedLabel
          hideIcon={false}
      ...
      {showChildrenItems && level <= maxLevel && getSubItems()}
    </div>
  );
};
